...
def batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum(x_mid * dout, axis=0)

    dx_hat = dout * gamma
    dvar = -0.5 * np.sum(dx_hat * (x - mean), axis=0) * (var + eps) ** -1.5
    dmu = -np.sum(dx_hat / np.sqrt(var + eps), axis=0) - 2 * dvar * np.sum(x - mean, axis=0) / N

    dx = dx_hat / np.sqrt(var + eps) + 2 * dvar * (x - mean) / N + dmu / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    normalized_dout = dout * gamma
    partial_var = np.sum(normalized_dout * (x - mean), axis=0)
    partial_std_inv = -0.5 * partial_var * (var + eps)**-1.5

    partial_mean = -np.sum(normalized_dout / np.sqrt(var + eps), axis=0)
    partial_mean -= 2 * partial_std_inv * np.sum(x - mean, axis=0) / N

    dx = normalized_dout / np.sqrt(var + eps)
    dx += 2.0 * partial_std_inv * (x - mean) / N
    dx += partial_mean / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = x, gamma, beta, x_normalized, mean, var, eps
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    window = x[n, c, h1:h2, w1:w2]
                    m = np.max(window)
                    dx[n, c, h1:h2, w1:w2] += (window == m) * dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    size = (N*G, C//G *H*W)
    x_reshaped_transposed = x.reshape(size).T

    gamma = gamma.reshape(1, C, 1, 1)
    beta = beta.reshape(1, C, 1, 1)

    mu = x_reshaped_transposed.mean(axis=0)
    variance = x_reshaped_transposed.var(axis=0) + eps
    std = np.sqrt(variance)

    z = (x_reshaped_transposed - mu)/std
...
def spatial_groupnorm_backward(dout, cache):
    ...
    z_reshaped_transposed = cache['z'].reshape(size).T
    M = z_reshaped_transposed.shape[0]
    dfdz = dout * gamma
    dfdz = dfdz.reshape(size).T
    dfdz_sum = np.sum(dfdz,axis=0)
    
    dx = dfdz - dfdz_sum/M - np.sum(dfdz * z_reshaped_transposed,axis=0) * z_reshaped_transposed/M
    dx /= std
