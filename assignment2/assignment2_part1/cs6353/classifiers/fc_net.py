...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, relu_cache_2 = affine_relu_forward(X2, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx2, dw2, db2 = affine_relu_backward(softmax_grad, relu_cache_2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            ...
            gamma = self.params.get(f'gamma{layer}', None)
            beta = self.params.get(f'beta{layer}', None)
            bn_params = self.bn_params[layer - 1] if self.normalization is not None else None
            ...
            bn_cache = None
            if self.normalization == 'batchnorm':
                norm_out, bn_cache = batchnorm_forward(affine_out, gamma, beta, bn_params)
            elif self.normalization == 'layernorm':
                norm_out, bn_cache = layernorm_forward(affine_out, gamma, beta, bn_params)
            else:
                norm_out = affine_out
            ...
            do_cache = None
            if self.use_dropout:
                final_out, do_cache = dropout_forward(relu_out, self.dropout_param)
            else:
                final_out = relu_out
        ...
        final_w, final_b = self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}']
        scores, final_cache = affine_forward(x, final_w, final_b)
        ...
        ############################################################################
        loss, softmax_grad = softmax_loss(scores, y)
        loss += 0.5 * self.reg * sum(np.sum(w * w) for w in self.params.values() if w.ndim == 2)
        ...
        grads[f'W{layers[-1]}'] = dw + self.reg * self.params[f'W{layers[-1]}']
        grads[f'b{layers[-1]}'] = db

        # Gradients for the remaining layers
        for layer in reversed(layers[:-1]):
            # Extracting the caches
            fc_cache, bn_cache, relu_cache, do_cache = caches[layer - 1]
